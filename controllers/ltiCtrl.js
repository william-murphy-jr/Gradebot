...
//   const fcc_includes = [ 'seed/challenges/02-javascript-algorithms-and-data-structures/basic-javascript.json', 'seed/challenges/01-responsive-web-design/basic-html-and-html5.json' ]
//   const fcc_index = {}
...
async function post (req, res) {
  ...
  provider.valid_request(req, async (err, isValid) => {
    if (err) {
      ...
      const assignmentId = req.body.custom_canvas_assignment_id || req.body.assignmentid
      const courseId = req.body.custom_canvas_course_id
      const userId = req.body.custom_canvas_user_id
      const submitted = await canvas.req(`/courses/${courseId}/assignments/${assignmentId}/submissions/${userId}`)
      const assignments_link = `/courses/${courseId}/assignments`
      let assignmnet
